...
# gets the first and last name for a patient from the user
# if the name is in the system, gets a new record line
# if the new record line is not blank, appends it to the
#  appropriate record file
def addRecord():
    firstName, lastName = getNames()
    ...
    if isPatientInSystem(firstName, lastName):
        recordFileName = getRecordFileName(firstName, lastName)
        newRecordLine = input("Enter a new line for " +\
                              firstName + " " + lastName +\
                              "'s medical record: ")
        if newRecordLine != "":        
            fileOut = open(recordFileName, 'a')
            fileOut.write(newRecordLine + "\n")
            fileOut.close()       
    else:
        print(firstName, lastName, "is NOT in the system.")
